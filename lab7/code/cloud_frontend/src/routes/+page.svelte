<script lang="ts">
	...
	import api, { sendMessageViaLambda } from '$lib/api';
	...
	const sendMessage = async () => {
		...
		try {
			...
			const textToProcess = newMessage || (selectedFile ? `[image: ${selectedFile.name}]` : '');
			if (textToProcess.trim()) {
				try {
					...
					console.error('Error sending message via Lambda:', lambdaError);
					lambdaStatus = `Lambda: blad — ${msg}`;
					setTimeout(() => (lambdaStatus = ''), 8000);
				}
			}
...
<div class="max-w-2xl mx-auto p-4">
	...
	<div class="border-t border-gray-200 pt-4 flex flex-wrap items-center gap-2">
		...
		{/if}
		{#if lambdaStatus}
			<span class="text-sm text-orange-600">{lambdaStatus}</span>
		{/if}
